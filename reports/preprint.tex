...
therefore the use of all-atom simulation may be unjustified.
...
experimental data.
We find that systematic problems limit the efficacy of the MARTINI potential
...
resolution simulation-dependent methods produce agreement that is only
slightly worse.
\end{abstract}
...
kinematic approach \cite{Crowley1991,Lu1996}.
...
calculation of the SLD profile from the simulation or the full determination
...
trichogin GA-IV within a supported lipid bilayer \cite{Bobone2013}.
...
all-atom \cite{Jambeck2012}, Berger united-atom \cite{Berger1997}, and
MARTINI coarse-grained potential models \cite{Marrink2007}.
...
previously published by Hollinshead \emph{et al.} \cite{Hollinshead2009},
and full details of the methods used can be found in the previous publication.
...
molecules \cite{Jambeck2012}, which was used alongside the single point
charge (SPC) water model \cite{Berendsen1987}, with a timestep
of \SI{0.5}{\femto\second}.
The Berger potential model is obtained by the integration of the hydrogen
atoms to produce a united atom potential model \cite{Berger1997}; again the
SPC water molecules were used.
...
MARTINI \cite{Marrink2007} alongside the polarisable MARTINI water
model \cite{Yesylevskyy2010}, to avoid the freezing issues observed
previously \cite{Koutsioubas2016}.
...
at \SI{300}{\kelvin} and a leap-frog integrator, and run using GROMACS 5.0.5 \cite{Berendsen1995,Lindahl2001,vanderSpoel2005,Hess2008} on 32 cores of
the STFC Scientific Computing resource SCARF.
...
software Packmol \cite{Martinez2009}.
...
A \SI{6}{\angstrom} layer of water was then added such that the head groups
were overlapping, this was achieved using the \texttt{solvate} functionality
in GROMACS 5.0.5.
...
\centering
  ...
  layer, visuallised using VMD\cite{Humphrey1996}.}
  \label{fig:drywet}
\end{figure}
%
%
...
\centering
  ...
  the work of Kubo \emph{et al.}\cite{Kubo2001}.}
  \label{fig:iso}
\end{figure}
%
%
...
\small
  ...
  \begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}lllll}
    ...
    \hline
    Average$\pm$Uncertainty & \input{../output/traditional/ave_30_chisq.txt} &
